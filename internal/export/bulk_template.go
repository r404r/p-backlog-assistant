...
var bulkGuideLines = [][2]string{
	...
	{"実行にかかる時間", "API 呼び出しは最低 1 秒間隔です。新規追加は 1 回、更新は競合確認を含め 2 回行うため、1,000 件では新規のみで最低約 17 分、更新のみで最低約 34 分が目安です。通信時間やレート制限による待機は別途加わります。"},
}
...
func writeBulkTemplateSheet(f *excelize.File, rows BulkTemplateSeq, masters BulkTemplateMasters) error {
	...
	sheet, err := newStreamDataSheet(f, SheetBulkTemplate, specs)
	if err != nil {
		return err
	}
	sw := sheet.writer
	...
	// 全データ行にドロップダウンが掛かる。
	return sheet.Finish(func() error { return addBulkDropDowns(f, masters, count) })
}
