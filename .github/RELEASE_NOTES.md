# backlog-assistant v1.0.7

サイドバーの使い勝手を改善するリリースです。

## 新機能

- **サイドバーの幅調整**: 右端のハンドルをドラッグして幅を調整できます(調整した幅は記憶され、再起動後も維持)
  - 一定幅より縮めると折りたたみ表示(短縮ラベル)に切り替わります。折りたたみ状態から右へドラッグすると展開します
  - ハンドルのダブルクリックで既定の幅に戻ります。従来の ≡ ボタンによる折りたたみ切替もそのまま使えます
